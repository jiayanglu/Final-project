...
```{r}
classification_fit <- train(Diabetes_binary ~ GenHlth + HighBP + DiffWalk + BMI + HighChol + Age + HeartDiseaseorAttack + PhysHlth + Income + PhysActivity,
...
From the output, we can choose the cp value that minimizes the chosen metric (logLoss) on the validation set, which is cp = 0.001. This cp value represents the optimal balance between model complexity and performance.
...
rf_fit <- train(Diabetes_binary ~ GenHlth + HighBP + DiffWalk + BMI + HighChol + Age + HeartDiseaseorAttack + PhysHlth + Income + PhysActivity,
                data = train_transformed_2,
                method = "rf",
                metric = "logLoss",
                trControl = trctrl,
