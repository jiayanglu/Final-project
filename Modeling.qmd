...
predicted_prob_classification <- predict(classification_fit,
                                         ...
                                         type='prob')
#convert variable Diabetes_binary from factor to numeric in order to use logLoss function
log_loss_classification <- logLoss(as.numeric(as.character(test_transformed_2$Diabetes_binary) == "Prediabetes_or_diabetes"),
...
From the result, we can see that the random forest tree model has the lowest log-loss value (0.3471412) among all the models, which means that the random forest tree model's predicted probabilities are cloest to the actual observed outcomes among all models. Therefore, the random forest tree model overall did the best job (in terms of log-loss metric) on the test set.